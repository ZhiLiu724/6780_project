...
for (K in 2:3){
  ...
  write.csv(mean.coda, file=paste(K,"_posterior.csv",sep=""), row.names = T)
  mat.dic <- as.matrix(multnorm.dic)
  write.csv(mat.dic, file=paste(K,"_dic.csv", sep=""),row.names = T)
}
...
p <- p + geom_point(aes(x=mu_x[k], y=mu_y[k]), size = 2, alpha=1.0, colour = "black")
p <- p + geom_ellipse(aes(x0=mu_x[k], y0=mu_y[k], a=a[k],b=b[k], angle = angle_x[k], color="black"))
